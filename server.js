...
let tvLive = false;
let radioLive = false;
let videoSource = 'webcam';
let youtubeSettings = { liveId: '', liveActive: false, playlist: ['', '', '', ''] };
function hasPlaylist() {
  return Array.isArray(youtubeSettings.playlist) && youtubeSettings.playlist.some(Boolean);
}
function fallbackToPlaylist() {
  if (hasPlaylist()) videoSource = 'youtube';
}
let obsSettings = {
  rtmpServer: process.env.RTMP_SERVER || 'rtmp://seu-dominio.com/live',
  streamKey: process.env.STREAM_KEY || 'acdsfm-live-123',
  playbackUrl: process.env.OBS_PLAYBACK_URL || '',
  ready: false
};
...
function emitStatus() {
  io.emit('status', {
    ...
    videoSource,
    obsReady: Boolean(obsSettings.ready && obsSettings.playbackUrl),
...
io.on('connection', (socket) => {
  ...
  socket.emit('youtube-settings', youtubeSettings);
  ...
  socket.on('admin-auth', (password, callback) => {
    ...
    socket.emit('obs-settings', obsSettings);
    socket.emit('youtube-settings', youtubeSettings);
    emitStatus();
  });


  socket.on('set-video-source', ({ source } = {}) => {
    if (socket.id !== adminSocketId) return;
    if (!['webcam', 'youtube', 'obs'].includes(source)) return;
    videoSource = source;
    emitStatus();
  });

  socket.on('update-obs-settings', (settings = {}) => {
    if (socket.id !== adminSocketId) return;
    obsSettings = {
      rtmpServer: String(settings.rtmpServer || obsSettings.rtmpServer).trim().slice(0, 160),
      streamKey: String(settings.streamKey || obsSettings.streamKey).trim().slice(0, 80),
      playbackUrl: String(settings.playbackUrl || obsSettings.playbackUrl).trim().slice(0, 220),
  ...
  socket.on('update-youtube-settings', (settings = {}) => {
    if (socket.id !== adminSocketId) return;
    const playlist = Array.isArray(settings.playlist) ? settings.playlist : youtubeSettings.playlist;
    youtubeSettings = {
      liveId: String(settings.liveId ?? youtubeSettings.liveId).trim().slice(0, 120),
      liveActive: Boolean(youtubeSettings.liveActive),
      playlist: [0,1,2,3].map(i => String(playlist[i] || '').trim().slice(0, 120))
    };
    io.emit('youtube-settings', youtubeSettings);
    emitStatus();
  });

  socket.on('start-youtube-live', ({ liveId } = {}) => {
    if (socket.id !== adminSocketId) return;
    const cleanLiveId = String(liveId || youtubeSettings.liveId || '').trim().slice(0, 120);
    if (!cleanLiveId) return;
    youtubeSettings.liveId = cleanLiveId;
    youtubeSettings.liveActive = true;
    videoSource = 'youtube';
    tvLive = true;
    io.emit('youtube-settings', youtubeSettings);
    emitStatus();
  ...
  socket.on('stop-youtube-live', () => {
    if (socket.id !== adminSocketId) return;
    youtubeSettings.liveActive = false;
    tvLive = false;
    fallbackToPlaylist();
  ...
  socket.on('volume-settings', (settings = {}) => {
    if (socket.id !== adminSocketId) return;
    io.emit('volume-settings', {
      musicVolume: Number(settings.musicVolume ?? 1),
      radioLiveVolume: Number(settings.radioLiveVolume ?? 1),
      tvLiveVolume: Number(settings.tvLiveVolume ?? 1)
    });
  });
